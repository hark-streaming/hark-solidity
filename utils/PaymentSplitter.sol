...
contract PaymentSplitter is Context {
    ...
    /**
     * @dev Triggers a transfer to each account, and then resets all shares to 0.
     */
    function releaseAllAndReset() internal virtual {
        // transfers to each account
        for(uint i = 0; i < _payees.length; i++) {
            release(payable(_payees[i]));
        }
        
        // nullifies each account
        for(uint i = 0; i < _payees.length; i++) {
            _shares[_payees[i]] = 0;
            _released[_payees[i]] = 0;
            emit PayeeRemoved(_payees[i]);
        }
